...
```{r}
library_usages <-
  ml_repo_imports[, date_floored:=floor_date(as.Date(release_date, origin = "1970-01-01"), "month")] %>%
  group_by(date_floored, import_name) %>% 
  summarize(count=n())
ggplot(library_usages, aes(fill=import_name, y=count, x=date_floored)) + 
  geom_bar(position="dodge", stat="identity") +
  ylim(0, max(library_usages$count))
